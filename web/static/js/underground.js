...
let Underground = {
  init: function(element, stations_link, store){
    if(!element){ return }
  ...
  update: function(element, stations_link){
    var getJSON = function(url, callback) {
      var xhr = new XMLHttpRequest();
      xhr.open("get", url, true);
      xhr.responseType = "json";
    ...
    getJSON(stations_link,
      function(err, stations) {
        if (err != null) {
          ...
          update_chart(element, stations)
        }
      }
    )

    function update_chart(element, stations){
      d3.csv('/csv/lines.csv', function(connections) {
        var stationsById = {};
